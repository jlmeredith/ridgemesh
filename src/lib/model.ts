...
export const MODEL_VERSION = 'ridgemesh-rf-2.0';
...
export type Environment = {crowd:number;bagLoss:boolean;clientsHop:boolean;meshHops:number;totemHops:number;communityTotems:boolean;fadeDb?:number;foliageDbPerM?:number;canopyM?:number;rootId?:string;meshRootId?:string;totemRootId?:string;mode?:'base'|'crew';crewIds?:string[];assumeUnknownHardware?:boolean;targetPolygon?:[number,number][]};
...
export function routeBetween(nodes:RadioNode[],fromId:string,toId:string,p:Environment,s:Surface,link=(a:RadioNode,b:RadioNode)=>directedLink(a,b,p,s)):Route{
 ...
 const best=new Map<string,number>();let unknownRoute:Route|undefined;
 for(let h=0;h<=limit;h++){const next:State[]=[];for(const q of frontier){if(q.node.id===toId){const result:Route={status:q.unknown?'unknown':q.margin<6?'marginal':'likely',route:q.route,marginDb:q.unknown?NaN:q.margin,hops:h,reason:q.unknown?'A required link has unknown inputs.':'Directed route; RF edge hop convention.'};if(!q.unknown)return result;unknownRoute=result;continue;}if(h===limit||(h>0&&!canForward(q.node,p)))continue;for(const n of nodes){if(q.route.includes(n.id))continue;const edge=link(q.node,n);if(!edge.ok&&edge.status!=='unknown')continue;const unknown=q.unknown||edge.status==='unknown',margin=unknown?-Infinity:Math.min(q.margin,edge.marginDb),key=`${h+1}:${n.id}:${unknown}`;if(best.has(key)&&best.get(key)!>=margin)continue;best.set(key,margin);next.push({node:n,route:[...q.route,n.id],margin,unknown});}}frontier=next.sort((a,b)=>Number(a.unknown)-Number(b.unknown)||b.margin-a.margin);}
 return unknownRoute??{status:'unavailable',route:[],marginDb:-Infinity,hops:0,reason:'No allowed route within the RF edge hop limit.'};
}
...
export function evaluateTarget(target:Target,nodes:RadioNode[],p:Environment,s:Surface,system:'mesh'|'totem',link?:(a:RadioNode,b:RadioNode)=>Budget):TargetResult{
 ...
 const ghost:RadioNode={...target,id:`target:${target.id}`,kind:target.kind??(system==='mesh'?'m1':'totem')};
 const peers=p.mode==='crew'?(p.crewIds??nodes.filter(n=>HARDWARE[n.kind].system===system&&n.kind!=='v4').map(n=>n.id)):[root??''];
