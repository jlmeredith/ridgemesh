...
export type Heat={scope?:'event'|'landscape';w:number;h:number;step:number;mesh:Float32Array;meshMargin:Float32Array;meshViaRelay:Uint8Array;pois:{mesh:TargetResult[]};modelVersion:string;targetWeights:Float32Array;targetAreaSqM:number};
...
export function computeHeat(nodes:Node[],p:SimParams,step=4,scope:'event'|'landscape'='event'):Heat{
 step=Math.max(4,Math.min(40,Math.round(step)));const w=Math.ceil((COLS-1)/step),h=Math.ceil((ROWS-1)/step),mesh=new Float32Array(w*h),meshMargin=new Float32Array(w*h),meshViaRelay=new Uint8Array(w*h);
 ...
 for(let gy=0;gy<h;gy++)for(let gx=0;gx<w;gx++){const target={id:`${gx}:${gy}`,label:'Roaming receiver',x:gx*step+Math.min(step,COLS-1-gx*step)/2,y:gy*step+Math.min(step,ROWS-1-gy*step)/2};const index=gy*w+gx;targetWeights[index]=weights.get(target.id)??0;if(targetWeights[index]===0&&scope!=='landscape')continue;const r=evaluateTarget(target,nodes,p,analysisSurface,'mesh',link);mesh[index]=code[r.status];meshMargin[index]=r.marginDb;meshViaRelay[index]=Number((r.status==='likely'||r.status==='marginal')&&(r.forward.route.length>2||r.reverse.route.length>2));}
 const pois={mesh:CAMPS.map((c,i)=>evaluateTarget({id:`camp-${i}`,label:c.label,x:c.x,y:c.y},nodes,p,analysisSurface,'mesh',link))};
 return {scope,w,h,step,mesh,meshMargin,meshViaRelay,pois,modelVersion:MODEL_VERSION,targetWeights,targetAreaSqM:targetWeights.reduce((a,b)=>a+b,0)};
}
export function coverageStats(heat:Heat,nodes:Node[]){const margins=Array.from(heat.meshMargin).filter((m,i)=>heat.targetWeights[i]>0&&Number.isFinite(m)&&heat.mesh[i]>0);const relayArea=Array.from(heat.meshViaRelay).reduce((sum,v,i)=>sum+(v?heat.targetWeights[i]:0),0);const frac=(test:(n:number)=>boolean)=>heat.targetAreaSqM?Array.from(heat.mesh).reduce((sum,v,i)=>sum+(test(v)?heat.targetWeights[i]:0),0)/heat.targetAreaSqM:0;return {targetAreaSqM:heat.targetAreaSqM,meshMinMarginDb:margins.length?Math.min(...margins):NaN,meshRelayFrac:heat.targetAreaSqM?relayArea/heat.targetAreaSqM:0,meshFrac:frac(v=>v>0),meshLikelyFrac:frac(v=>v===1),meshMarginalFrac:frac(v=>v===.5),meshUnavailableFrac:frac(v=>v===0),meshUnknownFrac:frac(v=>v<0),meshCamps:heat.pois.mesh.filter(r=>r.status==='likely'||r.status==='marginal').length,campCount:CAMPS.length,fixedCount:nodes.filter(isFixed).length,roamingCount:nodes.filter(n=>!isFixed(n)).length};}
export function analyzeScenario(nodes:Node[],params:SimParams,options:{step?:number;scope?:'event'|'landscape'}={}){const heat=computeHeat(nodes,params,options.step,options.scope);return {heat,stats:coverageStats(heat,nodes),pois:heat.pois,crewPairs:nodes.flatMap((a,i,crew)=>crew.slice(i+1).map(b=>({from:a.id,to:b.id,forward:routeBetween(nodes,a.id,b.id,params,analysisSurface),reverse:routeBetween(nodes,b.id,a.id,params,analysisSurface)})))};}
