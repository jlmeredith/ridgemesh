...
import { KIND_META, suggestBackbone, suggestClients, suggestTotems, type Heat, type Kind, type Node, type SimParams } from './radio';
...
export const DEFAULT_PARAMS: SimParams = { crowd: 0.35, bagLoss: false, clientsHop: false, meshHops: 3, totemHops: 5, communityTotems: false, mode: 'base', meshRootId: 'm1-0', totemRootId: 'totem-0' };
export type Scenario = { schema: 1; name: string; terrainVersion: string; modelVersion: string; nodes: Node[]; params: SimParams };
...
export function parseScenario(text: string, terrainVersion: string, modelVersion: string): Scenario {
  ...
  if (!d || typeof d.name !== 'string' || d.name.length > 100 || d.schema !== 1 || !Array.isArray(d.nodes) || d.nodes.length > 40) throw new Error('Expected a RidgeMesh v1 scenario with at most 40 nodes.');
  if (d.terrainVersion !== terrainVersion || d.modelVersion !== modelVersion) throw new Error('This scenario uses a different terrain or radio model version. Recreate it against the current model.');
...
export const useSim = create<State>((set,get)=>({
  ...DEFAULT_PARAMS, nodes:presetNodes('backbone'), overlay:'mesh', tool:'select', selected:null, probe:null, heat:null,
  name:'V4-assisted crew',extraParams:{mode:'base',meshRootId:'m1-0',totemRootId:'totem-0'},history:[],hydrated:false,
  ...
  replaceNodes:nodes=>set(s=>({nodes,history:[...s.history.slice(-29),s.nodes],selected:null,probe:null,heat:null})),
  addNode:(kind,x,y)=>{const s=get();if(s.nodes.length>=40)return;const id=crypto.randomUUID();s.replaceNodes([...s.nodes,{id,kind,x:clampX(x),y:clampY(y),label:`${KIND_META[kind].title} ${s.nodes.length+1}`}]);set({selected:id,tool:'select'});},
  moveNode:(id,x,y)=>get().updateNode(id,{x:clampX(x),y:clampY(y)}),
  updateNode:(id,patch)=>set(s=>({nodes:s.nodes.map(n=>n.id===id?{...n,...patch,id:n.id}:n),history:[...s.history.slice(-29),s.nodes],heat:null})),
  removeNode:id=>get().replaceNodes(get().nodes.filter(n=>n.id!==id)),
  select:selected=>set({selected}),setTool:tool=>set({tool}),setOverlay:overlay=>set({overlay}),
  ...
  loadPreset:name=>{get().replaceNodes(presetNodes(name));set({...DEFAULT_PARAMS,extraParams:{...DEFAULT_PARAMS},name:name==='backbone'?'V4-assisted crew':name,overlay:name==='totem-crew'?'totem':name==='hybrid'?'both':'mesh',tool:'select'});},
  ...
  undo:()=>set(s=>s.history.length?{nodes:s.history[s.history.length-1],history:s.history.slice(0,-1),selected:null,probe:null,heat:null}:{}),
  restore:d=>set({...d.params,extraParams:{...d.params},nodes:d.nodes,name:d.name,selected:null,probe:null,history:[],heat:null,hydrated:true}),
}));
