...
self.onmessage=(event:MessageEvent<WorkerRequest>)=>{const q=event.data,start=performance.now();try{
 if(q.kind==='plan'){
  ...
  const analysis=analyzeScenario(result.nodes,result.params,{step:4,scope:'landscape'});
  self.postMessage({id:q.id,kind:q.kind,result,analysis,elapsed:performance.now()-start});
 }else self.postMessage({id:q.id,kind:q.kind,result:analyzeScenario(q.nodes,q.params,{step:4,scope:'landscape'}),elapsed:performance.now()-start});
